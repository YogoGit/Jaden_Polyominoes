...
import edu.carroll.polyominoes.service.account.RegisterService
...
class RegisterController(private val registerService: RegisterService) {
     ...
     */
    @GetMapping("/register")
    fun registerGet(model: Model): String {
    ...
    fun registerPost(@Valid @ModelAttribute registerForm: RegisterForm, result: BindingResult): String? {
        ...
        if (!validateConfirmPassword(registerForm.rawPassword, registerForm.rawPasswordConfirm)) {
            result.rejectValue("rawPasswordConfirm", "error.passwordConfirm", "Passwords do not match")
            errorOccured = true
        }
        if (errorOccured) {
            return null
